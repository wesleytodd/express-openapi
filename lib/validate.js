...
module.exports = function makeValidatorMiddleware (options, schema) {
  let ajv
  let validate

  function makeValidator (schema) {
    const reqSchema = Object.assign({}, BASE_REQ_SCHEMA)
    ...
    schema.parameters && schema.parameters.forEach((p) => {
      switch (p.in) {
        case 'path':
          ...
          p.required && reqSchema.properties.params.required.push(p.name)
          break
        case 'query':
          reqSchema.properties.query.properties[p.name] = p.schema
          p.required && reqSchema.properties.query.required.push(p.name)
          break
        case 'header':
          const name = p.name.toLowerCase()
          reqSchema.properties.headers.properties[name] = p.schema
          p.required && reqSchema.properties.headers.required.push(name)
          break
      }
    })
